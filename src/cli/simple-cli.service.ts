...
export class SimpleCliService {
  ...
  private async handleStatusCommand(): Promise<void> {
    ...
    try {
      ...
      if (stockCount > 0) {
        ...
        if (latestData) {
          console.log(`🕐 Neueste Daten: ${latestData.stock.ticker} (${latestData.timestamp.toLocaleDateString()})`);
        }
      }
  ...
  private async handleTrackCommand(ticker: string): Promise<void> {
    ...
    try {
      ...
      await this.dataIngestionService.fetchHistoricalDataForStock(ticker.toUpperCase());
  ...
  private async handlePredictCommand(ticker: string): Promise<void> {
    if (!ticker) {
      console.log('❌ Bitte geben Sie ein Ticker-Symbol an: kairos predict AAPL');
      return;
    }
    ...
    try {
      ...
      if (!stock) {
        console.log(`❌ ${ticker} wird nicht verfolgt. Fügen Sie es zuerst hinzu: kairos track ${ticker}`);
        return;
      }      // Aktuelle Daten abrufen
      await this.dataIngestionService.fetchLatestDataForStock(ticker.toUpperCase());
      
      // Prognose erstellen
      const prediction = await this.mlPredictionService.predictNext(ticker.toUpperCase());
      
      if (prediction) {
        console.log('\n📊 Prognose-Ergebnis:');
        console.log('====================');
        console.log(`🎯 Aktie: ${ticker.toUpperCase()}`);
        console.log(`🎲 Konfidenz: ${(prediction.confidence * 100).toFixed(1)}%`);
        console.log(`💹 Prognostizierte Richtung: ${prediction.direction > 0 ? '📈 Aufwärts' : '📉 Abwärts'}`);
        console.log(`🕐 Zeitstempel: ${prediction.timestamp.toLocaleString()}`);
        
        if (prediction.targetPrice) {
          console.log(`💰 Prognostizierter Preis: $${prediction.targetPrice.toFixed(2)}`);
        }
  ...
  private async handleListCommand(): Promise<void> {
    ...
    try {
      ...
      if (stocks.length === 0) {
        ...
        console.log('💡 Verwenden Sie "kairos track <TICKER>" um eine Aktie hinzuzufügen');
        return;
      }
      ...
      for (const stock of stocks) {
        ...
        const date = latestData ? latestData.timestamp.toLocaleDateString() : 'N/A';
        
        console.log(`📈 ${stock.ticker.padEnd(6)} | ${stock.name.padEnd(30)} | ${price.padStart(10)} | ${date}`);
      }
    } catch (error) {
  ...
  private showHelp(): void {
    ...
    console.log('  train            Trainiert das ML-Modell mit aktuellen Daten');
