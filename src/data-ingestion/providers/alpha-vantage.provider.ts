...
export class AlphaVantageProvider implements DataProvider {
  ...
  async fetchHistoricalData(ticker: string, days: number = 365): Promise<MarketDataPoint[]> {
    ...
    try {
      ...
      this.logger.debug(`${dataPoints.length} historische Datenpunkte für ${ticker} von Alpha Vantage abgerufen`);
      return dataPoints;

    } catch (error) {
      this.logger.error(`Fehler beim Abrufen historischer Daten für ${ticker} von Alpha Vantage`, error);
      throw error;
    }
  }
  ...
  async fetchLatestData(ticker: string): Promise<MarketDataPoint | null> {
    ...
    try {
      ...
      if (!quote) {
        this.logger.warn(`Keine aktuellen Daten für ${ticker} von Alpha Vantage erhalten`);
        return null;
      }
      ...
      this.logger.debug(`Aktuelle Daten für ${ticker} von Alpha Vantage abgerufen`);
      return point;

    } catch (error) {
      this.logger.error(`Fehler beim Abrufen aktueller Daten für ${ticker} von Alpha Vantage`, error);
      throw error;
    }
  }
}
