...
export class FinnhubProvider implements DataProvider {
  ...
  async fetchHistoricalData(
    ...
    if (!this.isConfigured()) {
      this.logger.warn(
        'Finnhub Provider nicht konfiguriert - API-Schlüssel fehlt',
      );
      return [];
    }
    ...
    try {
      ...
      this.logger.debug(
        `Hole Finnhub-Daten für ${ticker} von ${fromDate} bis ${toDate}`,
      );
      ...
      if (data.s !== 'ok') {
        this.logger.warn(
          `Keine Daten für ${ticker} von Finnhub: Status ${data.s}`,
        );
        return [];
      }
      ...
      this.logger.debug(
        `${dataPoints.length} Datenpunkte für ${ticker} von Finnhub abgerufen`,
      );
      return dataPoints;
    } catch (error) {
      this.logger.error(
        `Fehler beim Abrufen historischer Daten für ${ticker} von Finnhub`,
        error,
      );
      throw error;
    }
  ...
  async fetchLatestData(ticker: string): Promise<MarketDataPoint | null> {
    ...
    try {
      ...
      if (!data.c) {
        this.logger.warn(
          `Keine aktuellen Daten für ${ticker} von Finnhub erhalten`,
        );
        return null;
      }
      ...
      this.logger.error(
        `Fehler beim Abrufen aktueller Daten für ${ticker} von Finnhub`,
        error,
      );
      throw error;
    }
