...
describe('PortfolioService', () => {
  ...
  describe('createPortfolio', () => {
    ...
    it('should create a portfolio with initial positions', async () => {
      ...
      jest.spyOn(service as any, 'calculatePortfolioReturns').mockResolvedValue([0.033]);
      ...
      const portfolio = await service.createPortfolio(portfolioName, initialPositions);
  ...
  describe('createPortfolioWithCapital', () => {
    it('should create a portfolio with initial capital', async () => {
      ...
      const portfolio = await service.createPortfolioWithCapital(portfolioName, initialCapital);
  ...
  describe('getPortfolio', () => {
    ...
    it('should return existing portfolio', async () => {
      ...
      const retrievedPortfolio = await service.getPortfolio(createdPortfolio.id);
  ...
  describe('addPosition', () => {
    ...
    beforeEach(async () => {
      portfolio = await service.createPortfolioWithCapital('Test Portfolio', 10000);
      
      // Mock required methods
      jest.spyOn(service as any, 'updatePortfolioMetrics').mockResolvedValue(undefined);
    });
    ...
    it('should throw error for non-existing portfolio', async () => {
      await expect(service.addPosition('non-existing', 'AAPL', 10, 150))
  ...
  describe('removePosition', () => {
    ...
    it('should throw error for non-existing portfolio', async () => {
      await expect(service.removePosition('non-existing', 'AAPL'))
        .rejects.toThrow('Portfolio non-existing nicht gefunden');
    });

    it('should throw error for non-existing position', async () => {
      await expect(service.removePosition(portfolio.id, 'MSFT'))
  ...
  describe('calculatePortfolioMetrics', () => {
    ...
    beforeEach(async () => {
      ...
      // Mock required methods
      jest.spyOn(service as any, 'getCurrentPrices').mockResolvedValue(
        new Map([['AAPL', 155], ['MSFT', 310]])
      );
      jest.spyOn(service as any, 'calculatePortfolioReturns').mockResolvedValue([0.01, 0.02, 0.033]);
      jest.spyOn(service as any, 'calculateSharpeRatio').mockReturnValue(1.2);
      jest.spyOn(service as any, 'calculateMaxDrawdown').mockReturnValue(0.05);
      jest.spyOn(service as any, 'calculateVolatility').mockReturnValue(0.15);
      jest.spyOn(service as any, 'calculateTotalReturn').mockReturnValue(0.033);
    ...
    it('should handle portfolio with no positions', async () => {
      ...
      jest.spyOn(service as any, 'getCurrentPrices').mockResolvedValue(new Map());
      jest.spyOn(service as any, 'calculatePortfolioReturns').mockResolvedValue([]);
  ...
  describe('assessPortfolioRisk', () => {
    ...
    beforeEach(async () => {
      portfolio = await service.createPortfolioWithCapital('Risk Test Portfolio', 10000);
      ...
      jest.spyOn(service as any, 'calculatePositionRisks').mockResolvedValue([]);
    });
    ...
    it('should identify high volatility risk', async () => {
      ...
      expect(['MEDIUM', 'HIGH', 'CRITICAL']).toContain(riskAssessment.riskLevel);
    });
