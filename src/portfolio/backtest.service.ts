...
export class BacktestService {
  ...
  async runBacktest(config: BacktestConfig): Promise<BacktestResult> {
    ...
    let positions: { [symbol: string]: { quantity: number; entryPrice: number; entryDate: Date } } = {};
    
    // Mock historical data generation for each symbol
    for (const symbol of symbols) {
      const historicalData = this.generateMockHistoricalData(symbol, startDate, endDate);
      
      for (const dataPoint of historicalData) {
        const signals = await this.generateTradingSignals(symbol, dataPoint, strategy.name);
        ...
        for (const signal of signals) {
          if (signal.action === 'BUY' && !positions[symbol]) {
            ...
            const holdingPeriod = Math.floor((signal.timestamp.getTime() - position.entryDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    for (const [symbol, position] of Object.entries(positions)) {
      ...
      const buyValue = position.quantity * position.entryPrice;
      const tradeReturn = sellValue - buyValue;
      const holdingPeriod = Math.floor((endDate.getTime() - position.entryDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    return this.calculateBacktestMetrics(trades, initialCapital, currentCapital, startDate, endDate, strategy.name);
  }

  async generateTradingSignals(ticker: string, dataPoint: StockData, strategyName: string): Promise<TradingSignal[]> {
  ...
  getPresetStrategies(): string[] {
    return ['RSI_STRATEGY', 'MACD_STRATEGY', 'BOLLINGER_BANDS', 'MOVING_AVERAGE_CROSSOVER'];
  }
  ...
  private generateMockHistoricalData(symbol: string, startDate: Date, endDate: Date): StockData[] {
  ...
  private calculateBacktestMetrics(
    ...
    const totalProfit = trades.filter(t => t.return > 0).reduce((sum, t) => sum + t.return, 0);
    const totalLoss = Math.abs(trades.filter(t => t.return < 0).reduce((sum, t) => sum + t.return, 0));
